...
export default function PagarCartela() {
  ...
  const handlePagbankPayment = async (method: 'pix' | 'CREDIT_CARD') => {
    ...
    try {
      ...
      if (e.message.includes('CPF_REQUIRED')) toast.error("CPF Inválido. Digite um CPF correto com 11 números.");
      else if (e.message.includes('401') || e.message.includes('FetchError')) toast.error("Sua conexão caiu. Recarregue a página e tente novamente.");
      else toast.error("Erro PagBank: " + e.message);
    } finally {
             ...
             <div className="space-y-4">
                ...
                <div className="space-y-3">
                   ...
                   <div className="bg-white p-4 rounded-xl border shadow-sm space-y-3">
                      ...
                      <Button className="w-full bg-blue-600 hover:bg-blue-700" onClick={() => handlePagbankPayment('CREDIT_CARD')} disabled={isCardLoading || valorBase <= 0}>
                         {isCardLoading ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : null} Pagar no Cartão
                      </Button>
                   </div>
