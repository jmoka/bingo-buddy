...
export const CreditRequestDialog = ({ gameSettings, children }: CreditRequestDialogProps) => {
  ...
  const cardFeeDetails = calcPagbankFee('card'); // <-- Aqui estava o erro! Restaurado.
  ...
  const handlePagbankPayment = async (method: 'pix' | 'CREDIT_CARD') => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        else if (method === 'pix' && data.qr_code) { setPagbankData({ qr_code: data.qr_code, qr_code_text: data.qr_code_text }); toast.success("PIX Gerado!"); }
      } else {
