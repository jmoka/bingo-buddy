...
export const CreditRequestDialog = ({ gameSettings, children }: CreditRequestDialogProps) => {
  ...
  const handleStripePayment = async () => {
    ...
    try {
      ...
      toast.error("Erro ao iniciar Stripe: " + e.message);
    } finally {
      setIsStripeLoading(false);
    }
  };
          ...
          <div className="space-y-4 pt-2">
             ...
             {gameSettings?.pagbank_enabled && (
                <div className="bg-white dark:bg-card p-4 rounded-xl border border-border shadow-sm space-y-3">
                   ...
                   <Button className="w-full h-12 bg-blue-600 hover:bg-blue-700 text-white font-bold shadow-button" onClick={() => handlePagbankPayment('CREDIT_CARD')} disabled={isStripeLoading || amount <= 0}>
             ...
             {gameSettings?.pagbank_enabled && (
                <div className="bg-white dark:bg-card p-4 rounded-xl border border-border shadow-sm space-y-3">
                   ...
                   {!pagbankData ? (
                     <>
                        {pixFeeDetails && <p className="text-[10px] text-muted-foreground bg-muted/50 p-2 rounded-lg border border-dashed">Inclui taxa de serviço de R$ {pixFeeDetails.fee.toFixed(2)}.</p>}
