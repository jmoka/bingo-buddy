-- Restaurar role do admin que foi acidentalmente alterado para 'vendedor'
-- Identifica perfis que estão como 'vendedor' mas possuem entrada em vendedores_rifa
-- criada via aprovação, E que originalmente deveriam ser admin.
-- ATENÇÃO: ajuste o email/id do seu usuário admin se necessário.
-- Execute o SELECT abaixo primeiro para confirmar qual usuário será afetado:
-- SELECT p.id, p.full_name, p.role FROM perfis p
-- JOIN vendedores_rifa v ON v.user_id = p.id
-- WHERE p.role = 'vendedor';

-- Após confirmar, rode o UPDATE substituindo o ID correto:
-- UPDATE perfis SET role = 'admin' WHERE id = '<SEU_USER_ID_AQUI>';

-- Correção da função aprovar_vendedor para não sobrescrever role de admin
