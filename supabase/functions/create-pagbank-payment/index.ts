...
serve(async (req) => {
  ...
  try {
    ...
    // Sobrescreve com metadados do frontend (Venda Avulsa)
    ...
    // O PagBank Checkout exige Nome e Sobrenome (Mínimo duas palavras). 
    ...
    try {
      if (type === 'venda_bingo' && metadata?.venda_id) {
          await supabaseAdmin.from('vendas_bingo_fisico').update({
              nome_comprador: finalName, telefone_comprador: metadata?.cliente_telefone || null
    ...
    } catch (err) { console.error("[create-pagbank-payment] Erro ao pre-salvar comprador:", err); }
    ...
    if (payment_method === 'CREDIT_CARD') {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Erro do PagBank no campo '${errParam}': ${errDesc}`);
        }
    ...
    // FLUXO DE ORDERS (PIX AUTOMÁTICO)
        ...
        if (!response.ok) {
            console.error("[create-pagbank-payment] Erro PagBank Order:", JSON.stringify(responseData));
            const errDesc = responseData.error_messages?.[0]?.description || "";
            const errParam = responseData.error_messages?.[0]?.parameter_name || "Desconhecido";
    ...
    console.error("[create-pagbank-payment] Fatal Error:", error.message);
